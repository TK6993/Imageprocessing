...
public class Perspective extends JPanel {
    ...
    void calculateNearestNeigbour(int srcPixels[], int srcWidth, int srcHeight, int dstPixels[], int dstWidth, int dstHeight, double degrees) {
    	...
    	for(int y = 0; y < dstHeight; y++) {
    		for(int x = 0; x < dstWidth; x++) {
    			...
    			//Verschieben 1
    			int  yp = y- dstHeight/2;
    			int  xp = x- dstWidth/2;
    			
    			// Persepektive
        		double ys= yp/(cos-0.001*sin*yp);
        		double xs= xp*(0.001*sin*ys+1);
        		int xn = (int)(Math.round(xs));
        		int yn = (int)(Math.rint(ys));
        		...
        		int newpos = yn* srcWidth +xn;
        		
        
        		
        		if(yn>=0&& yn<srcHeight && xn>=0 && xn<srcWidth)
        			dstPixels[pos]=srcPixels[newpos];
        		else{dstPixels[pos]=0xffffffff;}
    ...
    void calculateBilinear(int srcPixels[], int srcWidth, int srcHeight, int dstPixels[], int dstWidth, int dstHeight, double degrees) {
    	...
    	for(int y = 0; y < dstHeight; y++) {
    		for(int x = 0; x < dstWidth; x++) {
    			...
    			//perspektive
        		double yn= yp/(cos-0.001*sin*yp);
        		double xn= xp*(0.001*sin*yn+1);
        		...
        		if(yn>=0&& yn<srcHeight && xn>=0 && xn<srcWidth){
        				int yUnBound = (int)yn;
        				int yUpBound = (int)yn+1;
        				int xUnBound = (int)xn;
        				int xUpBound = (int)xn+1;
        				//Randbehandlung
        				if(yUpBound>=srcHeight)yUpBound--;
        				if(xUpBound>=srcWidth)xUpBound--;
        				double ydis = yn - (int)yn;
        				double xdis = xn - (int)xn;
        				int point1 = yUnBound* srcWidth+xUnBound;
        				int point2 = yUnBound* srcWidth+xUpBound;
        				int point3 = yUpBound* srcWidth+xUnBound;
        				int point4 = yUpBound*srcWidth+	xUpBound;        				
        				
        			int point13 =bulidAVGColorPoint(point1,point3,ydis,srcPixels);
        			int point24 =bulidAVGColorPoint(point2,point4,ydis,srcPixels);        					
        			int r =0;
        			int g =0;
        			int b =0;
        			
        			r+= (((point13 >> 16) & 0xff)*(1-xdis));
        			g+= (((point13 >> 8) & 0xff)*(1-xdis));
        			b+= ((point13 & 0xff)*(1-xdis));
        			r+= (((point24 >> 16) & 0xff)*(xdis));
        			g+= (((point24 >> 8) & 0xff)*(xdis));
        			b+= ((point24 & 0xff)*(xdis));
        			
        			dstPixels[pos] = (0xFF<<24) | ((int)r<<16) | ((int)g << 8) |(int)b;
    ...
    public int bulidAVGColorPoint(int p1, int p2, double distance,int[] srcP){
    	double r =0;
		double g = 0;
		double b = 0;
				r+= (((srcP[p1] >> 16) & 0xff)*(1-distance));
			    g+= (((srcP[p1] >> 8) & 0xff)*(1-distance));
			    b+= ((srcP[p1] & 0xff)*(1-distance));
			    r+= (((srcP[p2] >> 16) & 0xff)*(distance));
			    g+= (((srcP[p2] >> 8) & 0xff)*(distance));
			    b+= ((srcP[p2] & 0xff)*(distance));
		int ergebnis = (0xFF<<24) | ((int)r<<16) | ((int)g << 8) |(int)b;
		return ergebnis;
